...
async function checkGameFlowAndQueue() {
    if (!currentAccount || !currentAccount.auto_queue) return;
    ...
    try {
        ...
        if (phase === 'None') {
            ...
            const queueId = currentAccount.queue_type === 'RANKED_SOLO' ? 420 : 440;
            await lcu.request('POST', '/lol-lobby/v2/lobby', { queueId });
        } else if (phase === 'Lobby') {
            // 2. Set Roles (If in lobby)
            if (currentAccount.primary_role && currentAccount.secondary_role) {
                console.log("Setting Roles...");
                await lcu.request('PUT', '/lol-lobby/v2/lobby/members/localMember/position-preferences', {
                    firstPreference: currentAccount.primary_role,
                    secondPreference: currentAccount.secondary_role
                });
            }
            ...
            if (res) {
                ...
                currentAccount.auto_queue = false;
            }
        }
    } catch (e) {
    }
...
async function setAppearOffline() {
    if (currentAccount && currentAccount.appear_offline) {
        try {
...
lcu.onEvent(async (event) => {
    ...
    if (event.uri === '/lol-chat/v1/me' && event.eventType === 'Update') {
        ...
        if (currentAccount && currentAccount.appear_offline && event.data.availability !== 'offline') {
            setAppearOffline();
        }
    }
    ...
    if (currentAccount.auto_ban_champ) {
        const banAction = findMyAction('ban');
        if (banAction) {
            const champId = championMap[currentAccount.auto_ban_champ.toLowerCase()];
            if (champId) {
    ...
    if (currentAccount.auto_pick_champ) {
        const pickAction = findMyAction('pick');
        if (pickAction) {
            const champId = championMap[currentAccount.auto_pick_champ.toLowerCase()];
            if (champId) {
    ...
    // Auto Skin (Random)
    if (currentAccount.auto_skin_random) {
...
app.whenReady().then(async () => {
    ...
    setInterval(() => lcu.connect(config.lol_path), 5000);
...
ipcMain.handle('add-account', (event, data) => {
    ...
    accounts.push({
        ...
        region: data.region,
        auto_pick_champ: data.auto_pick_champ || "",
        auto_ban_champ: data.auto_ban_champ || "",
        notes: data.notes || "",
        auto_queue: data.auto_queue || false,
        queue_type: data.queue_type || 'RANKED_SOLO',
        primary_role: data.primary_role || '',
        secondary_role: data.secondary_role || '',
        appear_offline: data.appear_offline || false,
        auto_skin_random: data.auto_skin_random || false,
        auto_spells: data.auto_spells || false
    });
...
ipcMain.handle('update-account', (event, data) => {
    ...
    accounts[index] = {
        ...
        riot_id: data.riot_id || oldAcc.riot_id,
        region: data.region || oldAcc.region,
        auto_pick_champ: data.auto_pick_champ !== undefined ? data.auto_pick_champ : oldAcc.auto_pick_champ,
        auto_ban_champ: data.auto_ban_champ !== undefined ? data.auto_ban_champ : oldAcc.auto_ban_champ,
        notes: data.notes !== undefined ? data.notes : oldAcc.notes,
        auto_queue: data.auto_queue !== undefined ? data.auto_queue : oldAcc.auto_queue,
        queue_type: data.queue_type !== undefined ? data.queue_type : oldAcc.queue_type,
        primary_role: data.primary_role !== undefined ? data.primary_role : oldAcc.primary_role,
        secondary_role: data.secondary_role !== undefined ? data.secondary_role : oldAcc.secondary_role,
        appear_offline: data.appear_offline !== undefined ? data.appear_offline : oldAcc.appear_offline,
        auto_skin_random: data.auto_skin_random !== undefined ? data.auto_skin_random : oldAcc.auto_skin_random,
        auto_spells: data.auto_spells !== undefined ? data.auto_spells : oldAcc.auto_spells
    };
...
ipcMain.handle('launch-account', async (event, username) => {
    ...
    let launchCmd = `& "${config.lol_path}"`;

    // Improved Riot Client Discovery
    let riotClientPath = path.join(path.dirname(path.dirname(config.lol_path)), "Riot Client", "RiotClientServices.exe");
...
ipcMain.handle('get-stats', async (event, { region, riot_id }) => {
    if (!riot_id || !riot_id.includes('#')) return { tier: "N/A" };

    try {
        const [name, tag] = riot_id.split('#');
        const url = `https://www.op.gg/summoners/${region}/${encodeURIComponent(name)}-${tag}`;
            ...
            try {
                ...
                if (data) {
                    ...
                    if (data.league_stats && Array.isArray(data.league_stats)) {
                        const solo = data.league_stats.find(s => s.queue_info?.queue_translate === 'Ranked Solo/Duo');
                        if (solo && solo.tier_info) {
                            tier = `${solo.tier_info.tier} ${solo.tier_info.division}`;
                            lp = `${solo.tier_info.lp} LP`;
                            winLose = `${solo.win}W ${solo.lose}L`;
                            const total = solo.win + solo.lose;
                            if (total > 0) {
        ...
        if (tier === "Unranked") {
            ...
            if (tierEl.length) tier = tierEl.text().trim();
                ...
                if (header.length) {
                    ...
                    const match = content.match(/(Iron|Bronze|Silver|Gold|Platinum|Emerald|Diamond|Master|Grandmaster|Challenger)\s+(\d)/);
...
ipcMain.handle('change-language', async (event, locale) => {
    try {
        const gameDir = path.dirname(config.lol_path);
